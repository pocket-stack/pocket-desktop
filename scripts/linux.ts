...
if (process.platform !== "linux") {
  throw new Error("Pocket Desktop Linux builds run on Linux; use the CI artifact from macOS");
}
...
if (packageArchive) {
  ...
  console.log(`Pocket Desktop Linux distribution: ${archive}`);
}

if (buildOnly || packageArchive) {
  console.log(
    `Pocket Desktop: built linux-app System UI + ${receipt.applicationCount} applications + release host`,
  );
  process.exit(0);
}
