...
function chromeBinary(): string {
  ...
  if (!binary) {
    throw new Error("Pocket Desktop web smoke requires Chrome or Chromium (set CHROME_BIN)");
  }
  return binary;
}
...
try {
  ...
  console.log(
    `Pocket Desktop web smoke: Ready, Hero AppInstance composited, screenshot ${screenshotPath}`,
  );
  ...
  if (!screenshotTaken) {
    console.error("Pocket Desktop web smoke failed before the acceptance screenshot");
  }
}
